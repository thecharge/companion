...
function SessionList({
  sessions, idx, active, onSelect, onNew, onDel,
}: {
  sessions: Session[]; idx: number; active: boolean;
  onSelect: (i: number) => void; onNew: () => void; onDel: () => void;
}) {
  useInput((_ch, key) => {
    if (!active) return;
  ...
  return (
    <Box flexDirection="column" width={34} borderStyle="single" borderColor={active ? "cyan" : "gray"}>
      <Text bold color="cyan"> Sessions {active ? "[n=new d=del ↑↓]" : ""}</Text>
      ...
      {sessions.map((s, i) => (
        <Box key={s.id}>
          ...
          >
            {" "}<Text color={modeColor(s.mode)}>●</Text>{" "}
            {s.title.slice(0, 23).padEnd(23)}{String(s.message_count).padStart(3)}
          </Text>
        </Box>
      ))}
      <Box marginTop={1}>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="yellow" paddingX={1} marginBottom={1}>
      <Box justifyContent="space-between">
        <Text bold color="yellow">⚙ {task.agent} — {label}…</Text>
        <Text color="gray">{elapsed}s</Text>
      </Box>
      {task.tool    && <Text color="cyan">  ↳ tool: <Text bold>{task.tool}</Text></Text>}
      {task.thought && <Text color="gray" dimColor wrap="wrap">  ❝ {task.thought.slice(0, 110)}</Text>}
    </Box>
  );
}
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      {entries.slice(-6).map((e, i) => (
        <Text key={i} color="gray" dimColor>{e.ts} {e.text}</Text>
      ))}
    </Box>
  );
}
...
function ChatPane({
  session, messages, task, log: actionLog,
  streaming, active, wsConnected,
  ...
  useEffect(() => { setScroll(0); }, [messages.length, session?.id]);
  ...
  useInput((ch, key) => {
    ...
    if (ch === "/" && !focused) { setFocus(true); return; }
  ...
  const visibleMessages = [...messages].reverse().slice(scrollOffset, scrollOffset + VISIBLE).reverse();
    ...
    <Box flexDirection="column" flexGrow={1} borderStyle="single" borderColor={active ? "cyan" : "gray"}>
      {!session ? (
        ...
        <>
          {/* Header */}
          <Box justifyContent="space-between">
            <Text bold color="cyan"> {session.title.slice(0, 28)}</Text>
            <Box>
          ...
          {session.summary && (
            <Text color="gray" dimColor wrap="wrap"> ↻ {session.summary.slice(0, 120)}</Text>
          )}
            ...
            {visibleMessages.map((m) => (
              <Box key={m.id} flexDirection="column" marginBottom={1}>
                <Text bold color={m.role === "user" ? "green" : "white"}>
                  {m.role === "user" ? "You" : "Companion"}{m.streaming ? " ▌" : ""}
                </Text>
          ...
          <Box
            borderStyle="round"
            borderColor={focused ? "green" : streaming ? "yellow" : "gray"}
            marginTop={1}
          >
            {streaming ? (
...
function CapabilitiesPane({ caps, active }: { caps: Caps | null; active: boolean }) {
  ...
  return (
    <Box flexDirection="column" width={36} borderStyle="single" borderColor={active ? "cyan" : "gray"}>
      <Text bold color="cyan"> Capabilities {active ? "[1/2/3]" : ""}</Text>
      ...
      {caps && (
        <>
          ...
          {tab === "agents" && caps.agents.map((a) => (
            <Box key={a.name} flexDirection="column" marginBottom={1} marginLeft={1}>
              <Text bold>{a.name} <Text color="gray">({a.model})</Text></Text>
              <Text color="gray" wrap="wrap">  {a.description.slice(0, 100)}</Text>
            </Box>
          ))}
          {tab === "tools" && caps.tools.slice(0, 15).map((t) => (
            <Box key={t.name} flexDirection="column" marginBottom={1} marginLeft={1}>
              <Text bold>{t.name}</Text>
              <Text color="gray" wrap="wrap">  {t.description.slice(0, 80)}</Text>
            </Box>
          ))}
          {tab === "skills" && (caps.skills.length === 0
            ...
            : caps.skills.map((s) => (
              <Box key={s.name} flexDirection="column" marginBottom={1} marginLeft={1}>
                ...
                <Text color="gray" wrap="wrap">  {s.description}</Text>
              </Box>
            ))
          )}
        </>
...
function App() {
  ...
  // WebSocket
  const connectWS = useCallback((session: Session) => {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    const hardTimeout = setTimeout(() => { ctrl.abort(); addLog("timeout (120s)"); }, 120_000);
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          } catch { /* partial SSE line — next read completes it */ }
        }
      }
    } catch (e) {
  ...
  return (
    <Box flexDirection="column" height={process.stdout.rows ?? 40}>
      ...
      <Box justifyContent="space-between" paddingX={1}>
        <Text bold color="cyan">⚡ Companion</Text>
      ...
      <Box flexGrow={1}>
        <SessionList
          sessions={sessions} idx={idx} active={pane === "sessions"}
          onSelect={(i) => {
            setIdx(i);
